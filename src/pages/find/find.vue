...
<script lang="ts">
	...
	export default defineComponent({
		...
		setup() {
			...
			const popup = ref() 
			const searchkey = ref('')
			const swiperList = ref(['http://image.wufazhuce.com/FiHIrjKDFYrongr6RYOHs7mVqJ5f', 'http://image.wufazhuce.com/Fn7g1GKNvdKFIog6zbFG-dlzgeKM',
				'http://image.wufazhuce.com/FvchqyP1RX2-6A_uudu8CsIoH01_'])
			const swiperIndex = ref(0)//是否联网
			const scrollLeft = ref(0)
			const tabs = ref(['最新', 'Heco', 'BSC', 'OEC', 'ETH', 'Matic', 'TRON', 'TRON'])
			const selectedIndex = ref(0)
			const scrollHeight = ref(0)
			onLoad(()=>{
				let { statusBarHeight = 0, windowHeight } = uni.getSystemInfoSync()
				scrollHeight.value = windowHeight - statusBarHeight - uni.upx2px(100) - uni.upx2px(100)
			})
			...
			const listChange= (e)=> {
				selectedIndex.value = e.detail.current
				checkCor();
			}
